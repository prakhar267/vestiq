  ...
  c.html(
    shell(
      ...
      <p class="muted" style="max-width:60ch;margin-top:var(--s3)">
        ...
        automatically — no development work needed. Listing, feed syncing, and
        demand insights are all free during launch. No payment details are required.
      </p>
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(
      `INSERT INTO ${T.brands}
        (id, slug, name, domain, city, country, affiliate_rate_bp, status, created_at, updated_at)
       VALUES (?,?,?,?,?, 'IN', 0, 'pending', ?, ?)`,
  ...
  return c.html(
    layout(
      ...
      `<div class="wrap-narrow"><section class="section">
        ...
        <p class="muted">Listing, feed syncing, and demand insights are free during launch.</p>
...
merchantRoutes.get('/merchant', async (c) => {
  ...
  return c.html(
    shell(
      ...
      'Dashboard',
      `<h1>Dashboard</h1>
      <div class="notice good" style="margin-top:var(--s4)">
        Listing, feed syncing, and shopper demand insights are free during launch.
        No payment details or paid campaign are needed.
      </div>
      ${
        m.status !== 'approved'
...
merchantRoutes.get('/merchant/feed', async (c) => {
  ...
  return c.html(
    shell(
      ...
      'Feed',
      `<h1>Product feed</h1>
      <p class="muted" style="max-width:64ch">Automatic feed syncing and listing updates are free during launch.</p>
...
merchantRoutes.get('/merchant/demand', async (c) => {
  ...
  return c.html(
    shell(
      ...
      the useful half: searches in your categories where someone else got the click.
      These insights are free during launch.</p>
